...
`.github/workflows/validate.yml` é executado em `push` e `pull_request` para `main`, com permissão somente de leitura (`contents: read`). O workflow executa, nesta ordem:
...
`python scripts/build_site.py --check` verifica se os HTMLs publicados e o `404.html` estão sincronizados com suas fontes antes da regeneração. Em seguida, o primeiro `python scripts/build_site.py` regenera os artefatos e o primeiro `git diff --exit-code` falha caso essa regeneração produza qualquer diferença inesperada no checkout.

O gerador é então executado uma segunda vez. O segundo `git diff --exit-code` verifica a idempotência: sem alterações nas fontes, uma nova execução deve produzir exatamente o mesmo resultado e não modificar qualquer arquivo versionado. Por fim, `python scripts/validate_site.py` executa as validações estruturais do site.

Assim, uma alteração futura no menu/header/footer pode ser feita em uma única fonte e propagada para todas as páginas com `python scripts/build_site.py`, enquanto o CI verifica sincronização, determinismo, idempotência e integridade estrutural.
...
Esse fluxo permite que o CI funcione como gate técnico antes da publicação pelo GitHub Pages quando a proteção da `main` estiver efetivamente habilitada.
...
4. habilitar a exigência de branch atualizada antes do merge apenas se isso não introduzir atrito desnecessário;
...
Depois dessa configuração, um PR cujo job `validate` falhar, permanecer pendente ou estiver ausente quando obrigatório não deverá poder ser integrado pelo fluxo normal de merge.
